...
2. Copies the generated executable into `$flutterDir/assets/` folder.
...
3. To test gRPC Web locally you can use gRPC-Web-Proxy binary like that:
 ...
 5. Manually start the generated Python binary from Flutter's `./assets` folder to test it for any issues (what if it crashes).
...
Web clients can't work over HTTP2 and require a proxy in front of gRPC server. As of 2023, there is no work in progress on a Python proxy (Sonora doesn't work with Dart client). The 2 options are Envoy suggested by Google and [gRPC Web](https://github.com/improbable-eng/grpc-web).
...
- When using VSCode and having multiple Python installed, make sure you pick the right one (same used for running scripts and for debugging/editing). I.e., if default system Python interpreter is different from the one picked in VSCode, you can get all sorts of IDE issues, such as warnings from PyLace, failed debugging etc.:
  - Click Python version at the bottom (<img width="162" alt="268485002-c35c413d-37e6-4593-80c6-3a3075eb55ca-2" src="https://github.com/maxim-saplin/flutter_python_starter/assets/7947027/3c8b1a04-86bc-49ea-b5dc-abf4dee06aaf">), in the popup window, check that you got the right Python (recomended in my case was not chosen).
